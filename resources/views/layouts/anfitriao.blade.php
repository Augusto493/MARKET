...
<head>
    ...
    <title>@yield('title', ($header ?? 'Anfitrião') . ' - ' . config('app.name'))</title>
    ...
    <div class="min-h-screen flex">
        ...
        <aside class="hidden lg:flex lg:flex-shrink-0 lg:flex-col w-64 bg-slate-900 border-r border-slate-700">
            <div class="flex items-center gap-2 h-16 px-6 border-b border-slate-700">
                ...
                <span class="text-white font-semibold">Anfitrião</span>
            </div>
            <nav class="flex-1 py-4 overflow-y-auto">
                <ul class="space-y-0.5 px-3">
                    <li><a href="{{ route('anfitriao.dashboard') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('anfitriao.dashboard') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">Dashboard</a></li>
                    <li><a href="{{ route('anfitriao.conectar-stays') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('anfitriao.conectar-stays*') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">Conectar Stays</a></li>
                    <li><a href="{{ route('anfitriao.imoveis') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('anfitriao.imoveis') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">Meus imóveis</a></li>
                    <li><a href="{{ route('anfitriao.reservas') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium {{ request()->routeIs('anfitriao.reservas') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">Reservas</a></li>
                </ul>
            </nav>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            <header class="bg-white border-b-2 border-slate-300 shadow-sm flex-shrink-0">
                <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
                    <h1 class="text-xl font-bold text-slate-900">{{ $header ?? 'Área do anfitrião' }}</h1>
